...
void Sensors::CmdVelCallback(const geometry_msgs::msg::Twist &msg)
{
  ...
  // msg.linear.y ignoré : impossible physiquement avec des roues standards
  sensorSignals->SetInverseKinematics(x, w);
}
...
void Sensors::JoyCallback(const sensor_msgs::msg::Joy &msg)
{
  ...
  // Left Stick = XY
  // APRÈS
  // Stick gauche axe vertical = avance/recul
  float x = msg.axes[1] * maximumVelocity;
  if (msg.axes[1] == 0)
  {
    // D-pad vertical en secours
    x = msg.axes[7] * maximumVelocity;
  }
  // Stick gauche axe horizontal = rotation (pas de déplacement latéral)
  float w = msg.axes[0] * maximumVelocity * 2.0f;
  if (msg.axes[0] == 0)
  {
    // D-pad horizontal en secours
    w = msg.axes[6] * maximumVelocity * 2.0f;
  }
  // LT/RT ne servent plus pour la rotation, tu peux les réaffecter si besoin
  sensorSignals->SetInverseKinematics(x, w);
}
...
void Sensors::Publish(const McuData& mcuData)
{
  ...
  jointState.name = {"wheel_left_joint", "wheel_right_joint"};  // adapter selon ton URDF
  jointState.position = {motorsPosition[0], motorsPosition[1]};
  jointState.velocity = {mcuData.motors_actual_velocity[0], mcuData.motors_actual_velocity[1]};
