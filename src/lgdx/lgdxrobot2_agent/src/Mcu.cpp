...
// APRÈS
void Mcu::SetInverseKinematics(float x, float w)
{
  // Paramètre à adapter selon la géométrie de ton robot
  const float wheelBase = 0.25f; // distance entre les deux roues en mètres — À MESURER

  float leftVelocity  = x - (w * wheelBase / 2.0f);
  float rightVelocity = x + (w * wheelBase / 2.0f);

  McuMotorCommand cmdLeft;
  cmdLeft.header1  = MCU_HEADER1;
  cmdLeft.header2  = MCU_HEADER2;
  cmdLeft.command  = MCU_MOTOR_COMMAND_TYPE;
  cmdLeft.motor    = 0;  // moteur gauche
  cmdLeft.velocity = leftVelocity;
  std::vector<char> bufLeft(sizeof(McuMotorCommand));
  std::memcpy(bufLeft.data(), &cmdLeft, sizeof(McuMotorCommand));
  Write(bufLeft);

  McuMotorCommand cmdRight;
  cmdRight.header1  = MCU_HEADER1;
  cmdRight.header2  = MCU_HEADER2;
  cmdRight.command  = MCU_MOTOR_COMMAND_TYPE;
  cmdRight.motor    = 1;  // moteur droit
  cmdRight.velocity = rightVelocity;
  std::vector<char> bufRight(sizeof(McuMotorCommand));
  std::memcpy(bufRight.data(), &cmdRight, sizeof(McuMotorCommand));
  Write(bufRight);
}
